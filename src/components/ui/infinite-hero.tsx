...
export default function InfiniteHero() {
	...
	useGSAP(
		() => {
			...
			gsap.set(bgRef.current, { filter: "blur(28px)", opacity: 0.8 });
			...
			tl.to(bgRef.current, { filter: "blur(0px)", opacity: 1, duration: 1.5 }, 0)
				.to(
					h1Ref.current,
					{
						opacity: 1,
				...
				.to(ctas, { opacity: 1, y: 0, duration: 0.6, stagger: 0.1 }, 1);
		},
		{ scope: rootRef },
	);
		...
		>
			...
			<div className="relative z-10 flex h-svh w-full items-center justify-center px-6">
				<div className="text-center">
					<h1
						ref={h1Ref}
						className="mx-auto max-w-3xl lg:max-w-5xl text-[clamp(2.5rem,7vw,5rem)] font-bold leading-[1.05] tracking-tight"
					>
					...
					>
						<button
							type="button"
							className="group relative overflow-hidden bg-amber-500 hover:bg-amber-400 px-8 py-3 text-base rounded-full font-semibold tracking-wide text-black transition-all duration-500 hover:shadow-[0_0_40px_rgba(246,168,0,0.4)] cursor-pointer"
						>
						...
						<button
							type="button"
							className="group relative px-8 py-3 text-base font-medium tracking-wide text-white/90 border border-white/20 rounded-full transition-all duration-500 hover:border-amber-500/50 hover:text-amber-400 hover:bg-amber-500/10 cursor-pointer"
						>
							View Projects
						</button>
					</div>
